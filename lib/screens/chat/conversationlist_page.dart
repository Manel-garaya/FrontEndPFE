  ...
  Future<void> _fetchContacts() async {
    try {
      final response = await http.get(
          Uri.parse('http://192.168.1.33:8085/api/users/contacts/${widget.id}'));
    ...
    for (Contact contact in contacts) {
      try {
        final response = await http.get(Uri.parse(
            'http://192.168.1.33:8085/api/users/imageByEmail/${contact.email}'));
        if (response.statusCode == 200) {
    ...
    for (Contact contact in contacts) {
      try {
        final response = await http.delete(Uri.parse(
            'http://192.168.1.33:8085/api/users/removeContacts/${widget.id}/${contact.email}'));
        if (response.statusCode == 200) {
          setState(() {
...
class ChatUserWidget extends StatelessWidget {
 ...
 final String? image;
  ...
  Future<String> _fetchContactId(String email) async {
    try {
      final response = await http.get(
          Uri.parse('http://192.168.1.33:8085/api/users/idByEmail?email=$email'));
      if (response.statusCode == 200) {
